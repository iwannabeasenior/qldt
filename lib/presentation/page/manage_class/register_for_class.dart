...
class RegisterForClass extends StatelessWidget {
...
class RegisterForClassView extends StatefulWidget {
  const RegisterForClassView({super.key});

  @override
  State<RegisterForClassView> createState() => _RegisterForClassViewState();
}
// class _RegisterForClassViewState extends State<RegisterForClassView> {
...
class _RegisterForClassViewState extends State<RegisterForClassView> {
  ...
  void _submitRegistration() async {
    ...
    await provider.registerClass("41xI4u", selectedClassIds).then((_) {
      ...
      setState(() {
        registeredClasses.addAll(filteredClassData.where((classInfo) =>
            selectedClassIds.contains(classInfo.classId)));
      });
    }).catchError((e) {
      // Show error message if there's an issue
      ScaffoldMessenger.of(context).showSnackBar(
